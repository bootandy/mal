...
fn eval(
    mut tokens: Vec<reader::Token>,
    enviro: &mut HashMap<reader::Token, reader::Token>,
) -> Vec<reader::Token> {
    //vec![eval::apply_sym_multi(reader::Token::List(vec![]), &mut tokens, enviro)]
    vec![
        eval::apply_sym_single(&eval::_remove_or_nil(&mut tokens), enviro),
    ]
}
...
fn test_complex_let() {
    ...
    let mut enviro: HashMap<reader::Token, reader::Token> = HashMap::new();
    assert_eq!(
        rep("(let* (p (+ 2 3) q (+ 2 p)) (+ p q))", &mut enviro),
        "12"
    );
    assert_eq!(
        rep("(let* (a 5 b 6) [3 4 a [b 7] 8])", &mut enviro),
        "[3 4 5 [6 7] 8]"
    );
}
...
fn test_closure_fn() {
    let mut enviro: HashMap<reader::Token, reader::Token> = HashMap::new();
    assert_eq!(
        rep("( ( (fn* (a) (fn* (b) (+ a b))) 5) 7)", &mut enviro),
        "12"
    );
}
...
fn test_recursive_fib() {
    let mut enviro: HashMap<reader::Token, reader::Token> = HashMap::new();
    rep(
        "(def! fib (fn* (a) (if (<= a 1) 1 (+ (fib(- a 1)) (fib(- a 2))) )))",
        &mut enviro,
    );
